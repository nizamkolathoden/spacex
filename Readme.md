...
cd spacex
```
## wait if you didn't install docker in your local mechine first of all install it
...
### in this project i used mongodb atlas so i didn't use mongodb image in docker 
```zsh
DB="your mongoDB atlas API"
PORT=8080
...
## Now Everything is ok let's run our backend app and scale docker 

```zsh
    docker-compose up --scale node-app=5
```
## voila
...
eg url:
```
localhost:80/single/rocket/629ecfc82eb137f004d58e43
```
...
eg:
```
localhost:80/all/rockets?status=Failed
```
...
eg:
```
localhost/all/rockets?page=1&size=3
```
